...
void Network::initializeNetwork(int numInputs, std::vector<LayerInfo> hiddenLayers, int numOutputs) {
    // First layer
    Layer firstLayer = Layer(hiddenLayers[0].numNeurons, numInputs + 1, hiddenLayers[0].activationFunction);
    addLayer(firstLayer);

    // Hidden layers
    for (int i = 0; i < hiddenLayers.size() - 1; i++) {
        Layer hiddenLayer = Layer(hiddenLayers[i + 1].numNeurons, hiddenLayers[i].numNeurons + 1, hiddenLayers[i + 1].activationFunction);
        addLayer(hiddenLayer);
    }

    // Last layer
    Layer lastLayer = Layer(numOutputs, hiddenLayers.back().numNeurons + 1, hiddenLayers.back().activationFunction);
    addLayer(lastLayer);
    setNumLayers();
}
...
void Network::insertHiddenLayer(LayerInfo layer) {
    Layer output = layers.back();
    // Remove output layer
    layers.pop_back();
    // Add hidden layer
    layers.push_back(Layer(layer.numNeurons, layers.back().getNeurons().size() + 1, layer.activationFunction));
    // Add output layer
    layers.push_back(Layer(output.getNeurons().size(), layers.back().getNeurons().size() + 1, output.getActivationFunction()));
    setNumLayers();
}
...
void Network::removeHiddenLayers(int num) {
    ...
    newLayers.push_back(Layer(output.getNeurons().size(), layers[lastHiddenLayer].getNeurons().size() + 1, output.getActivationFunction()));
