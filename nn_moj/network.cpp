...
*/
void Network::initialize_network(int n_inputs, std::vector<std::pair<int, int>> n_hidden, int n_outputs) {

    // add the first hidden layer (n_hidden neurons are each connected to all inputs)
    this->add_layer(n_hidden[0].first, n_inputs+1, n_hidden[0].second);

    for (int i = 0; i < n_hidden.size()-1; i++) {
        // add a hidden layer (n_hidden neurons are each connected to all neurons from last layer)
        this->add_layer(n_hidden[i+1].first, n_hidden[i].first+1, n_hidden[i+1].second);
    }


    // add an output layer (one neuron for each output is connected to all neurons from the previous layer)
    this->add_layer(n_outputs, n_hidden.back().first+1, n_hidden.back().second);
}

/*
* Add another layer to the network
*/
void Network::add_layer(int n_neurons, int n_weights, int activation_function) {
    m_layers.push_back(Layer(n_neurons, n_weights, activation_function));
    m_nLayers++;
}
